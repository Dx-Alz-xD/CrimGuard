...
//
//   GET  /api/files/shared                files shared with this account, and the clipboard watch list
...
function registerHoneytrapRoutes(router, { sessions, crimguard, honeytrap }) {
  router.get('/api/files/shared', async ({ req, res }) => {
    const user = sessions.requireUser(req);
    const [files, watch] = await Promise.all([honeytrap.filesFor(user), honeytrap.watchList(user)]);
    sendJson(res, 200, { files, watch });
  });
